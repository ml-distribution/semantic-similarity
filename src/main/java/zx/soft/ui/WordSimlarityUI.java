...
import zx.soft.similarity.word.cilin.Cilin;
import zx.soft.similarity.word.cilin.CilinDb;
import zx.soft.similarity.word.hownet2.concept.BaseConceptParser;
import zx.soft.similarity.word.hownet2.concept.Concept;
import zx.soft.similarity.word.hownet2.concept.LiuConceptParser;
...
public class WordSimlarityUI extends JFrame {
	...
	public static JPanel createPanel() {
		...
		viewWordsInCodeButton.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				String text = "具有编码" + cilinField.getText() + "的词语有:"
						+ CilinDb.getInstance().getCilinWords(cilinField.getText());
				text += "\n__________________________________\n";
				text += result.getText();
				result.setText(text);
				result.setCaretPosition(0);
		...
		viewCodeButton.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				String text = cilinField.getText() + "的编码为:"
						+ CilinDb.getInstance().getCilinCoding(cilinField.getText());
				text += "\n__________________________________\n";
				text += result.getText();
				result.setText(text);
				result.setCaretPosition(0);
